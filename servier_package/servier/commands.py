...
def preprocess_data(data, model_type):
    ...
    if model_type == 'Model_1':
        data['smiles'] = data['smiles'].apply(
            lambda x: fingerprint_features(x).flatten()
            )
        data['smiles'] = data['smiles'].apply(
...
@click.option("--model_type", default=True, type=click.STRING,
              help="Whether training Model_1 or Model_2")
...
def predict(x, model_path, model_type):
    ...
    Prints the index of the prediction of the output row.

    Arguments:\n
        [x] the file that holds the example that should be predicted
    """
